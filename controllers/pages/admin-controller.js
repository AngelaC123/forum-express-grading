...
const adminController = {
  ...
  createRestaurant: (req, res, next) => {
    return Category.findAll({
      ...
      .then(categories => res.render('admin/create-restaurants', { categories }))
      .catch(err => next(err))
  },
  ...
  putRestaurant: (req, res, next) => {
    const { name, tel, address, openingHours, description, categoryId } = req.body
    if (!name) throw new Error('Restaurant neam is requied!')
    ...
    Promise.all([
      ...
      .then(() => {
        req.flash('success_messages', 'restaurant has update successfully!')
        res.redirect('/admin/restaurants')
      })
      .catch(err => next(err))
  },
